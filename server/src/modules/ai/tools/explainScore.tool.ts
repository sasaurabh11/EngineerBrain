...
// Maintainability is a whole-file Halstead/complexity formula, not a per-finding
// penalty tally like the other scores - QUALITY findings are the closest
// concrete drivers a reader can inspect, so they're surfaced as "related" here.
...
export const explainScoreTool: AiTool<ExplainScoreArgs> = {
  name: "explain_score",
  description:
    "Returns a repository's health score (overall/architecture/security/performance/maintainability) along with the specific findings that drove it, so you can explain WHY the score is what it is rather than giving a generic definition.",
  ...
  async execute(args, ctx: ToolContext) {
    ...
    const findings = await analysisRepository.listFindings(analysis.id, category ?? undefined);
